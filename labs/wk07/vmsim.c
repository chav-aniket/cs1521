...
int physicalAddress(uint vAddr, char action)
{
	uint page_num = vAddr/ nPages;
	uint offset = vAddr % nPages;
	int frame_no = -1;
	int pAddress = 0;
	...
	if (PageTable[page_num].status.loaded) {
		if (action == 'W')
			PageTable[page_num].status.modified = 1;
		PageTable[page_num].lastAccessed = clock;
		pAddress = PageTable[page_num].frameNo + offset;
	} else {
		...
		if (frame_no != -1) {
			MemFrames[frame_no] = pAddress;
		} else {
			int first_time;
			for (int i = 0; i < nPages; i++) {
				if (PageTable[i].lastAccessed < first_time) {
					frame_no = i;
					first_time = PageTable[i].lastAccessed;
				}
			}
			PageTable[frame_no].status.loaded = 0;
			nReplaces++;
			if (PageTable[frame_no].status.modified)
				nSaves++;
			MemFrames[frame_no] = pAddress;
		}
		nLoads++;
		PageTable[page_num].frameNo = frame_no;
		PageTable[page_num].lastAccessed = clock;
