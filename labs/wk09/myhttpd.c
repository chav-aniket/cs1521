	...
	int *n_requests, int *n_responses)
{
	...
	char var1[BUFSIZ];
	char var2[BUFSIZ];
	int n = sscanf(request, "GET /%[^ ?]?%s", var1, var2);
	// printf("var2 is %s\n", var2);
	// printf("var1 is %s\n", var1);
	time_t rawtime;
	time(&rawtime);

	char def[26] = "<h2>myhttpd running!</h2>";
	char hello[16] = "<h2>Hello!</h2>";
	char name[23] = "<h2>Hello, ";
	strcat(name, var2);
	strcat(name, "</h2>");
	char date[BUFSIZ] = "<h2>";
	strcat(date, ctime(&rawtime));
	strcat(date, "</h2>");
	char nonexistent[28] = "<h2>404 Page Not Found</h2>";
	char *result;
	...
	if (n == 0) {
		...
		if (strcmp(var1, "hello") == 0) {
			result = hello;
		} else if (strcmp(var1, "date") == 0) {
			result = date;
		} else if (strcmp(var1, "nonexistent") == 0) {
			result = nonexistent;
		} else {
			result = def;
		}
